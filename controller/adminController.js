...
import { productfilter, productfilterTag } from "../service/adminSevice.js";
...
export const ProductList = async (req, res) => {
    const { category, type,price } = req.query;
        
    try {
      const { products, total } = await productfilter(category, type,price);
...
export const ProductListTag = async (req, res) => {
  const { tag } = req.query;
      
  try {
    const { products, total } = await productfilterTag(tag);
    if (products.length > 0) {
      return res.status(200).json({
        status: success.successTrue,
        message : success.datadetail,
